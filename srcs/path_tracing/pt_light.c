...
/*   Updated: 2026/02/07 11:52:17 by maanguit         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color	direct_light(t_scene *scene, t_hit hit, t_dir wo)
{
	...
	if (!scene->lights->defined)
		return ((t_color){0.0, 0.0, 0.0});
	ldir = vec_sub(scene->lights->point, hit.p);
	...
	if (in_shadow(scene, hit, ldir, dist))
		return ((t_color){0.0, 0.0, 0.0});
	radiance = vec_x_scalar(scene->lights->color, scene->lights->intensity);
